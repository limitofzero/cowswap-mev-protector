...
use crate::{
    enemies::components::EnemyType,
    game::GameState,
    resources::PauseState,
...
fn setup_menu(
    ...
    let col_btn_bg  = materials.add(ColorMaterial { color: Color::srgba(0.40, 0.28, 0.80, 0.95), alpha_mode: AlphaMode2d::Blend, ..default() });
    let col_btn_fill = materials.add(ColorMaterial { color: Color::srgba(0.22, 0.12, 0.55, 0.97), alpha_mode: AlphaMode2d::Blend, ..default() });
    ...
    commands.spawn((
        ...
        // Panel border + fill
        p.spawn((Mesh2d(border), MeshMaterial2d(col_border), Transform::from_xyz(0.0, 0.0, 0.0)));
        p.spawn((Mesh2d(fill),   MeshMaterial2d(col_fill),   Transform::from_xyz(0.0, 0.0, 0.05)));
...
fn update_btn_label(
    ...
    let label = if *state == GameState::Menu { "Start" } else { "Proceed" };
...
fn handle_menu_input(
    ...
    if *state != GameState::Menu && !pause.paused { return; }
    ...
    let Ok(overlay_t) = overlay_q.single() else { return };
