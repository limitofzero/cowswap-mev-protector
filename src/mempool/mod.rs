...
use crate::{game::GameState, resources::{not_paused, NetworkLoad}};
...
#[derive(Component)] struct MempoolDot { progress: f32, speed: f32 }
#[derive(Component)] struct MempoolPathBase;
#[derive(Component)] struct MempoolPathShim;
...
fn setup_scene(
    ...
    for i in 0..waypoints.len() - 1 {
        ...
        // Base layer
        commands.spawn((
            Sprite { color: PATH_BASE_COLORS[0], custom_size: Some(Vec2::new(len, BASE_W)), ..default() },
        ...
        // Shim layer
        commands.spawn((
            Sprite { color: PATH_SHIM_COLORS[0], custom_size: Some(Vec2::new(len, SHIM_W)), ..default() },
    ...
    for &pt in waypoints {
        commands.spawn((
            ...
            MeshMaterial2d(materials.add(ColorMaterial { color: PATH_BASE_COLORS[0], ..default() })),
            Transform::from_xyz(pt.x, pt.y, -3.0),
        ));
        commands.spawn((
            Mesh2d(shim_cap.clone()),
            MeshMaterial2d(materials.add(ColorMaterial { color: PATH_SHIM_COLORS[0], ..default() })),
...
fn update_mempool_colors(
    ...
    for mut s in &mut base_q { s.color = PATH_BASE_COLORS[lv]; }
    for mut s in &mut shim_q { s.color = PATH_SHIM_COLORS[lv]; }
